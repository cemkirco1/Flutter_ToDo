...
import 'package:flutterapp11/arkaplan.dart';
...
import 'package:flutterapp11/yardimci/tarih_yardimi.dart';
import 'package:flutterapp11/sayfa/detaylar.dart';
import 'package:flutterapp11/bilesen/todo_rozet.dart';
import 'package:flutterapp11/sayfa/gizlilik_politikasi.dart';
import 'package:flutterapp11/model/data/choice_card.dart';
import 'package:flutterapp11/rota/olcek.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ScopedModel<TodoListModel>( //liste modellerini çağırıyoruz
      model: TodoListModel(),
      child: app,
    );
  }
...
class MyHomePage extends StatefulWidget {
  ...
  HeroId _generateHeroIds(Task task) {             //modelleri çağırma
    return HeroId(
  ...
  String currentDay(BuildContext context) {
    ...
    return DateTimeUtils.currentDay;   //tarih çağırma
  }

  @override
  _MyHomePageState createState() => _MyHomePageState();
}
class _MyHomePageState extends State<MyHomePage>  //geçişler
  ...
  Widget build(BuildContext context) {
    return ScopedModelDescendant<TodoListModel>(
        builder: (BuildContext context, Widget child, TodoListModel model) {
          ...
          if (!_isLoading) {
            // Animasyon değerini yalnızca yükleme tamamlandığında üste taşı
            _controller.forward();
          }
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: <Widget>[
                          // ShadowImage(), iler resim eklenebilr
                          Container(
                            child: Text(
                          ...
                          Text( //TARİH YAZISININ AYARLARI
                            '${DateTimeUtils.currentDate} ${DateTimeUtils.currentMonth}',
                            style: Theme.of(context).textTheme.title.copyWith(
                                color: Colors.white.withOpacity(0.7)),
                          ),
                          Container(height: 16.0),
                          Text(  //kaç adet listeye sahip olduğumuzu yazan yazı ve renklendirmesi
                            'Siz ${_todos.where((todo) => todo.isCompleted == 0).length} adet listeye sahipsiniz',
                            // 
                    ...
                    Expanded(//sayfa kontrollerini ayarlar
                      key: _backdropKey,
                      flex: 1,
...
class AddPageCard extends StatelessWidget {
  ...
  const AddPageCard({Key key, this.color = Colors.black}) : super(key: key);
   ...
   @override
  Widget build(BuildContext context) {  //liste kardlarını düzenleme
    return Card(
